var Grid = function (settings) {
    ...
    var DivSelector, DivDataSelector, TableId, TableDataId, SourceUrl, Data, Sorting, Searching, Modal, ModalTitle,
        fnRowDisplayFormat, fnRowElementsAction, fnRowCreated, fnGridCreated,Selector,
    ...
    this.Binding = function() {
        ...
        $ajaxCall(url, data).success(function (response) {
            if (response == null || !response.success) {
                ...
                var selectp = "> table > tbody";
                if(_this.Selector){
                    selectp = _this.Selector;
                }

                for (var i = 0; i < result.length ; i++) {
                    var row = "";
                    var d = result[i];
                    var selectordiv = "#" + TableDataId + selectp;
                    if (DivDataSelector != undefined) {
                        var selectordiv = DivDataSelector + selectp;
                    }
